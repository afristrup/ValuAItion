...
def get_data(
    ...
    **kwargs,
    ...
    # Try reading with default engine first, fallback to python engine if needed
    try:
...
def handle_missing_values(
    df: pd.DataFrame, split: str, reduce_zip: bool
) -> pd.DataFrame:
    ...
    df.loc[missing_rebuilding, "REBUILDING_YEAR"] = df.loc[
        missing_rebuilding, "CONSTRUCTION_YEAR"
    ]
...
def groupby_mean_impute(
    df: pd.DataFrame, split: str, groupby_col: str, impute_col: str
) -> pd.DataFrame:
...
def transform_values(
    ...
    if reduce_municipality:
        df["MUNICIPALITY"] = df["MUNICIPALITY_CODE"].apply(municipality_code_mapper)
...
def __calculate_street_price_sqm(
    df: pd.DataFrame, split: str, run_id: str, calculate_street_price_sqm: bool
) -> pd.DataFrame:
    ...
    # during test: load the average sqm price for each street code from the train phase
        ...
        street_mean_sqm_price = pd.read_csv(
            file_path, index_col="STREET_CODE"
        ).squeeze()
    df["STREET_CODE_MEAN_SQM_PRICE"] = df["STREET_CODE"].map(street_mean_sqm_price)
    df["STREET_CODE_MEAN_SQM_PRICE"] = df["STREET_CODE_MEAN_SQM_PRICE"].fillna(
        df["SQM_PRICE"].mean()
    )
    return df
